...
% All the parameters are optimized to obtain high power under white
% noise
...
xd = myIntervalDiscretize(x', floor(sqrt(n*3)));
yd = myIntervalDiscretize(y', floor(sqrt(n*3)));
MI_e_ = mi(xd',yd');

% Mutual Information (Discretization Equal-Frequency)
xd = myQuantileDiscretize(x', floor(sqrt(n*3)));
yd = myQuantileDiscretize(y', floor(sqrt(n*3)));
MI_ef_ = mi(xd',yd');
...
% HSIC 
p = 0.04; % percentage of the euclidean median
