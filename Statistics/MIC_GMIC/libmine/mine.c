...
double mine_gmic(mine_score *score)
{
  ...
  double GMIC_1 = 0;
  int Z = 0;
  double** C_star = (double**)malloc(score->n*sizeof(double*));
  for (i=0; i<score->n; i++){
	C_star[i] = (double*)malloc(score->m[i]*sizeof(double));
  }
  for (i=0; i<score->n; i++){
    for (j=0; j<score->m[i]; j++){
      maxCkl = 0;
