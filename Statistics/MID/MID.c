...
double mid(double* x, double*y, long int n){
     ...
     long int i;
	 double idim_x, idim_y, idim_xy, mid_ret;

	 //Simone copy values of x and y
	 ...
	 // end, otherwise it was modifying the originals
	  ...
	  // MAIN PART
	  idim(x_new, y_new, n, level_max, level_max_cov, &idim_x, &idim_y, &idim_xy);
/*
	  printf("%f\n",idim_x);
	  printf("%f\n",idim_y);
	  printf("%f\n",idim_xy);
	  */
	  mid_ret = idim_x + idim_y - idim_xy;
	  if (mid_ret > 1) mid_ret = 1;
	  if (mid_ret < 0) mid_ret = 0;
